...
export function ConsentManagementDashboard() {
  ...
  const loadConsentData = async () => {
    try {
      ...
      // ###desabilitado_mvp### const allKeys = await spark.kv.keys()
      ...
      // ###desabilitado_mvp### const data = await spark.kv.get<ConsentRecord>(key)
        if (data && data.id) {
  ...
  const loadAuditData = async () => {
    try {
      // ###desabilitado_mvp### const auditTrail = await spark.kv.get<AuditLogEntry[]>('audit-trail') || []
  ...
  const revokeConsent = async (consent: ConsentRecord) => {
    try {
      ...
      // ###desabilitado_mvp### await spark.kv.set(`consent-${consent.sessionId}`, revokedConsent)
      ...
      // ###desabilitado_mvp### const existingAudit = await spark.kv.get<AuditLogEntry[]>('audit-trail') || []
