...
export function CriticalAudioAlerts({ onNavigateToSession, onAcknowledgeAlert }: CriticalAudioAlertsProps) {
  ...
  const playAlertSound = useCallback(async (alertType: CriticalAlert['alertType'], severity: CriticalAlert['severity']) => {
  ...
  }, [alertSettings.enabled, alertSettings.criticalAlertSound, generateAlertTone])

  // Handle repeat alert logic
  const handleRepeatAlert = useCallback((alert: CriticalAlert) => {
    let repeatCount = 0
    const repeatTimer = setInterval(() => {
      ...
      const currentAlert = criticalAlerts.find((a: CriticalAlert) => a.id === alert.id)
  ...
  }, [alertSettings.maxRepeats, alertSettings.repeatInterval, criticalAlerts, playAlertSound])
  ...
  useEffect(() => {
    ...
    activeAlertsRef.current.forEach(alertId => {
      const alert = criticalAlerts.find((a: CriticalAlert) => a.id === alertId)
  ...
  }, [criticalAlerts, alertSettings, playAlertSound, handleRepeatAlert])

  // Cleanup timers on unmount
  useEffect(() => {
    const timersRef = playbackTimersRef.current
    return () => {
  ...
  const acknowledgeAlert = (alertId: string) => {
    const updatedAlerts = criticalAlerts.map((alert: CriticalAlert) => 
  ...
  const muteAlertsTemporarily = (minutes: number) => {
    ...
    const updatedSettings = { ...alertSettings, muteUntil }
        ...
        {isSettingsOpen && (
          <div className="space-y-4 p-3 bg-muted/30 rounded-lg mb-4">
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(enabled) => {
                  const updatedSettings = { ...alertSettings, enabled }
                  setAlertSettings(updatedSettings)
                }}
              />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm">Volume2</span>
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const updatedSettings = { ...alertSettings, volume }
                      setAlertSettings(updatedSettings)
                    }}
