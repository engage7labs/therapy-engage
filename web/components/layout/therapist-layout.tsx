...
export default function TherapistLayout({ 
  ...
  const getNavigationItemClasses = (item: any) => {
    ...
    if (item.urgent) return 'text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20'
    return 'text-muted-foreground hover:text-foreground hover:bg-secondary'
  }

  const getActionButtonClasses = (action: any) => {
    if (action.primary) return 'bg-primary text-primary-foreground hover:bg-primary/90'
  ...
  const navigationItems = [
    ...
    { id: 'emergency', label: 'Emergency Protocols', icon: AlertTriangle, urgent: true },
  ]

  const quickActions = [
    { id: 'start-session', label: 'Start Session', icon: Video, primary: true },
    { id: 'emergency-call', label: 'Emergency Contact', icon: Phone, urgent: true },
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-card border-r border-border transition-all duration-300 flex flex-col`}>
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navigationItems.map((item) => (
            <button
              key={item.id}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${getNavigationItemClasses(item)}`}
            >
          ...
          <div className="p-4 border-t border-border">
            ...
            <div className="space-y-2">
              {quickActions.map((action) => (
                <button
                  key={action.id}
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${getActionButtonClasses(action)}`}
                >
