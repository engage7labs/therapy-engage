...
export function useWebRTCRecording(): WebRTCRecordingHook {
  ...
  const processRecording = useCallback(async () => {
    ...
    try {
      ...
      // ###desabilitado_mvp### const aiInsights = await spark.llm(prompt, 'gpt-4o', true)
      
      setSession(prev => prev ? {
        ...prev,
        status: 'completed',
        transcription: mockTranscription,
        aiInsights: {} // MVP: dados simulados
      } : null)
  ...
  const endSession = useCallback(async () => {
    ...
    if (finalSession) {
      ...
      const currentSessions = completedSessions as any[]
      setCompletedSessions([...currentSessions, finalSession] as any)
    ...
    return null
  }, [isRecording, stopRecording, cleanup, session, duration, setCompletedSessions, completedSessions])
